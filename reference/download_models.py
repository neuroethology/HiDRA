...
def missing(models_dir=MODELS_DIR):
...
def require_weights(models_dir=MODELS_DIR):
...
def download(repo=DEFAULT_REPO, revision=DEFAULT_REVISION, models_dir=MODELS_DIR, force=False):
    try:
...
def main():
    ...
    if args.check:
        absent = missing(args.models_dir)
        if absent:
            print(f"{len(absent)} of {len(REQUIRED)} weight file(s) missing from {args.models_dir}:")
            for f in absent:
                print(f"  {f}")
            sys.exit(1)
        print(f"All {len(REQUIRED)} weight files present in {args.models_dir}")
        return

    download(args.repo, args.revision, args.models_dir, args.force)
