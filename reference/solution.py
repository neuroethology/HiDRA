...
class Epoch:
    ...
    def enqueue_worker(self):
        if self.shuffle:
            rng = np.random.default_rng([self.epoch_idx, self.seed])
...
class SimpleEpoch:
    ...
    def __init__(self, dataset, rows, epoch_idx, shuffle=True):
        ...
        if shuffle:
            rng = np.random.default_rng([epoch_idx, dataset.seed])
            rng.bit_generator.state = rng.bit_generator.jumped().state
...
class Dataset(BaseDataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.epoch_seed = [0] + self.seed
        self.worker_seed = [1] + self.seed
    ...
    def augment(self, element, ctx):
        ...
        rng = np.random.default_rng([ctx["element_idx"], ctx["epoch_idx"]] + self.worker_seed)
...
working_dir = "/dev/shm/data/tmp"
persist_dir = os.path.join(project_dir, "models")
