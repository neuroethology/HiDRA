...
import predict as hidra_predict          # vid_of / enum_heads / load_metadata / discover
...
def cmd_train(args):
    ...
    for i, cfg in enumerate(configs, 1):
        ...
        cmd = [sys.executable, os.path.join(PKG, "train_perlab_heads.py"), "--config", cfg]
        if args.smoke:
            cmd.append("--smoke")
        r = subprocess.run(cmd, env=env, cwd=PKG)
